...
const sounds = {
  ...
  success: new Audio('Sound/Success small.MP3') // ★ 이 줄을 추가하세요
};
...
function checkForGoalAchievement(type, oldScore, newScore) {
  ...
  goals.forEach(goal => {
    // 1. 목표 도달 (완료 체크 안 된 것만)
    // 점수가 오르면서(old < goal) 목표 이상이 되었을 때(new >= goal)
    if (!goal.completed && oldScore < goal.score && newScore >= goal.score) {
      showGoalPanel(goal);
    }
    
    // 2. 점수 하락 시 자동 숨김
    // 현재 떠있는 목표보다 점수가 낮아지면 닫기
    if (currentActiveGoal === goal && newScore < goal.score) {
      hideGoalPanel();
    }
  });
}
...
// 목표판 보여주기
function showGoalPanel(goal) {
  currentActiveGoal = goal;
  
  const panelMsg = document.getElementById('goal-panel-message');
  const checkbox = document.getElementById('goal-complete-checkbox');
  
  // 메시지 및 체크박스 초기화
  panelMsg.textContent = goal.message;
  checkbox.checked = false; // 새로 떴을 땐 체크 해제 상태

  // 소리 재생
  if(sounds.success) {
      sounds.success.currentTime = 0;
      sounds.success.play().catch(e => console.log('Sound play error', e));
  }

  // 화면 분할 애니메이션 시작
  document.body.classList.add('goal-active');
}

// 목표판 숨기기
function hideGoalPanel() {
  currentActiveGoal = null;
  document.body.classList.remove('goal-active');
}

// [이벤트 리스너 추가]

// 1. 목표 완료 체크박스
document.getElementById('goal-complete-checkbox').addEventListener('change', function(e) {
  if (e.target.checked && currentActiveGoal) {
    // 완료 처리
    currentActiveGoal.completed = true;
    saveToStorage(); // 영구 저장
    
    // 잠시 후 닫기
    setTimeout(hideGoalPanel, 500);
  }
});

// 2. 닫기(X) 버튼
document.getElementById('goal-close-btn').addEventListener('click', hideGoalPanel);
